...
namespace PLSC::Demo
{
	Window::Window(const i32 width, const i32 height, b vsync)
		: Window(width, height, 0, 0, vsync)
	{
	}
	Window::Window(const i32 width, const i32 height)
		: Window(width, height, 0, 0, false)
	{
	}
...
#endif
		m_window = SDL_CreateWindow("PLSC",
			pos_x,
			pos_y,
			m_width,
			m_height,
